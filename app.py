...
with tab1:
    ...
    moteur_ia = st.selectbox("Choisissez le moteur IA", ["OpenAI", "Hugging Face", "Groq"])
...
Utilise toutes les informations et documents fournis. Si besoin, complète avec des données publiques récentes sur la ville ou le pays. Mets les sous-titres en gras. Rédige chaque section de façon détaillée et professionnelle.
        ...
        if moteur_ia == "OpenAI":
            ...
            with st.spinner("Génération IA Groq en cours..."):
                rapport = generate_groq_report(prompt, groq_api_key)
...
with tab3:
    ...
    if st.button("Envoyer"):
        if question.strip():
            # Utilise le moteur IA sélectionné pour le chatbot aussi
            if "GROQ_API_KEY" in st.secrets:
                groq_api_key = st.secrets["GROQ_API_KEY"]
                try:
                    reponse_ia = generate_groq_report(question, groq_api_key, model="llama3-8b-8192")
                except Exception as e:
                    reponse_ia = f"Erreur Groq : {e}"
            else:
                client = openai.OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
                try:
                    response = client.chat.completions.create(
